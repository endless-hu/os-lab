...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- 
        var ret: ptr to Thread
          threadMgrLck.Lock()
          while freeList.IsEmpty() 
            aThreadBecameFree.Wait(&threadMgrLck)
          endWhile
          ret = freeList.Remove()
          ret.status = JUST_CREATED
          threadMgrLck.Unlock()
          return ret
        endMethod
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- 
          ...
          aThreadBecameFree.Broadcast(&threadMgrLck)
          threadMgrLck.Unlock()
        endMethod
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        -- until one is available.
        --
        var ret: ptr to ProcessControlBlock
...
-----------------------------  InitFirstProcess ------------------------------
  ...
  function StartUserProcess(unused: int)
    var 
      pcb: ptr to ProcessControlBlock
      file: ptr to OpenFile
      initUserStackTop, initUserPC, initSystemStackTop: int

      currentInterruptStatus = DISABLED
      -- // allocate a new PCB
      pcb = processManager.GetANewProcess()

      -- // connect it with the thread
      ...
      /* compute and set the initial value for the user level stack */
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
        var i, f, frameAddr: int
          ...
          -- Find `numFramesNeeded` free frames and allocate them
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      var 
        ...
        print("\nfilename = ")
        */
        -- retrieve the file name from the virtual address into strBuffer
        ret = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
        if ret < 0
          return -1  -- // the file name is too long, return error (-1)
        endIf
        /*
        -- print(&strBuffer)
        -- nl()
        */
        ...
        /* The following steps are quite like InitFirstProcess() */
        ...
        -- // check if we fail to load an executable file
        if initUserPC < 0
          frameManager.ReturnAllFrames(&newAddrSpace)
          return -1
        endIf

        -- // recycle previous resources
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
    var 
      ...
      ret = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
      if ret < 0
        FatalError("Handle_Sys_Create fails to get filename!")
      endIf
      print(&strBuffer)
      nl()
      return 4000
